...
def _predict_user(model, req):
    ...
    #res['user'] = user
    #res['item'] = res.index #['index']
    #_logger.info('results in _predict_user function in batch folder')
    #_logger.info('user: %d', user)
    #_logger.info(res)
    #_logger.info('res info below')
    #_logger.info(res.info())
    #_logger.info('past info')
    #res = pd.DataFrame({'user': user, 'item': res.index, 'prediction': res.values})
    #res = pd.DataFrame({'user': user, 'item': res[['index']], 'prediction': res[['prediction']]})
    #res = pd.DataFrame({'user': user, 'item': res.item, 'prediction': res.prediction, 'prediction2': res.var})
    #
    _logger.info('%s produced %f/%d predictions for %s in %s', model, res.prediction.notna().sum(), len(udf), user, watch)
    return res


def predict(algo, pairs, *, n_jobs=None, **kwargs): # change n_jobs=None 
    """
    ...
    """
    #_logger.info('calling predict function in _predict file')
